...
<?php
    ...
    if(isset($_GET['del'])){
		$id = $_GET['del'];
		mysqli_query($db,"DELETE FROM examroutine Where id =$id");
	header('location: uploadexamroutine.php');
	}
?>
<?php
...
{    
 ...
 {
  $sql="INSERT INTO examroutine (file,type,size,date) VALUES('$final_file','$file_type','$new_size','$date')";
  mysqli_query($db,$sql);
  ?>
  <script>
  alert('successfully uploaded');
        window.location.href='uploadexamroutine.php?success';
...
<div class="container" style="max-width:700px;margin:0px auto;">
	<h2>  Upload exam routine here</h2>
 ...
 ?>
	...
	<script type="text/javascript">$(document).ready(function(){
        ...
        <table border="1" cellpadding="0" class="table table-bordered table-hover" cellspacing="0" id="myTable" class="display" width="100%">
				...
				<?php
				$db = mysqli_connect("localhost","root","","project");
				$sql="SELECT id,file,date FROM examroutine";
		        ...
		        <td><a class="btn btn-danger" href="uploadexamroutine.php?del=<?php echo $row['id'];?>">DELETE</a></td>
